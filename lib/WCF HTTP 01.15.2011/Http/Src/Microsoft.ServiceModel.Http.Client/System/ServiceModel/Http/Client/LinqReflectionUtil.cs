...
namespace System.ServiceModel.Http.Client
{
    ...
    internal static class LinqReflectionUtil
    {
        private static readonly Dictionary<MethodInfo, SequenceMethod> methodMap;
        private static readonly Dictionary<SequenceMethod, MethodInfo> inverseMap;

        // Initialize method map
        [System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Performance", "CA1810:InitializeReferenceTypeStaticFieldsInline",
            Justification = "This map only gets initialized once.")]
        static LinqReflectionUtil()
        {
            ...
            methodMap = new Dictionary<MethodInfo, SequenceMethod>(EqualityComparer<MethodInfo>.Default);
            inverseMap = new Dictionary<SequenceMethod, MethodInfo>(EqualityComparer<SequenceMethod>.Default);
            foreach (MethodInfo method in GetAllLinqOperators())
            {
                ...
                {
                    methodMap.Add(method, sequenceMethod);
                    inverseMap[sequenceMethod] = method;
                }
            }
        }
        ...
        internal static bool TryIdentifySequenceMethod(MethodInfo method, out SequenceMethod sequenceMethod)
        {
            method = method.IsGenericMethod ? method.GetGenericMethodDefinition() :
                method;
            return methodMap.TryGetValue(method, out sequenceMethod);
        }
